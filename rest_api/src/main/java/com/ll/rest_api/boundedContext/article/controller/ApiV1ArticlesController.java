...
public class ApiV1ArticlesController {
    ...
    public RsData<Article> delete(@PathVariable Long id, @AuthenticationPrincipal User user) {
            Member member = memberService.findByUsername(user.getUsername()).orElse(null);
            if(member == null) return RsData.of("F-1", "해당 user name으로 된 회원이 없습니다");
            ...
            RsData<Article> DeleteRs = articleService.delete(opArticle.get());
